...
export class RulesetManagementController extends Controller {
  ...
  @Response<errorResponse>('401', 'Unauthorized')
  public async getRuleset(@Path() rulesetID: string): Promise<Ruleset> {
    const r = await rita.getRuleset(rulesetID);
    if (!r) {
      throw new NotFoundError(
        `The ruleset with id '${rulesetID}' could not be found!`
      );
    }
    return r;
  }
  ...
  public async saveRuleset(
    ...
    const lRitaVersion = ritaVersion || "0.0.0"
    const ritaRange = `${major(lRitaVersion)}.0.0 - ${major(
      lRitaVersion
    )}.${minor(lRitaVersion)}.*`;
    if (ruleset.version && !satisfiesVersion(ruleset.version, ritaRange))
    ...
    if (ruleset.id && ruleset.id !== rulesetID)
      throw new UsageError(
        'If specified, id in the body must match the url parameter'
      );
    ...
    const r: Ruleset = new Ruleset(
      rulesetID,
      data.name,
      PersistentRita.parser.parseRuleSet(data)
    );
  ...
  @Security('api_key', ['manage'])
  @Response<errorResponse>('401', 'Unauthorized')
  public async deleteRuleset(@Path() rulesetID: string): Promise<void> {
    this.setStatus(204);
