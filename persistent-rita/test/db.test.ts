...
for (const db of dbs) {
    ...
    describe('test ' + db.name, function () {
        function checkSavedRulesetAmount(expected: number) {
            it(
                'Check that there are ' + expected + ' rulesets saved',
                async () => {
        ...
        describe('Setup', function () {
            it('Initiate Rita', async () => {
                ...
                expect(
                    () => (rita = new PersistentRita(edb, logger))
                ).not.toThrow();
        ...
        describe('Test save and load', function () {
            ...
            it('Save empty ruleset2', async () => {
                await rita.saveRuleset(
                    new Ruleset('er2', 'Empty Ruleset2', [])
                );
            });

            it('Save empty ruleset3', async () => {
                await rita.saveRuleset(
                    new Ruleset('er3', 'Empty Ruleset3', [])
                );
            });
            ...
            it('update empty ruleset', async () => {
                await rita.saveRuleset(
                    new Ruleset('er2', 'Empty Ruleset x', [], 'description')
                );
            });
            ...
            it('Save exampleMath', async () => {
                await rita.saveRuleset(
                    new Ruleset(
                        'math',
                        'Math Ruleset',
                        parser.parseRuleSet(exampleMath)
                    )
                );
            });
            ...
            it('add 4 rules', async () => {
                for (let i = 0; i < 4; i++) {
                    await rita.saveRuleset(
                        new Ruleset('er' + i, 'Empty Ruleset', [])
                    );
                }
