...
export default abstract class LocalCVCSolver extends BaseSolver {
    ...
    protected parseOutput(stdout : bl.LineStream, errback : (reason ?: any) => void, callback : (value : (PromiseLike<SatResult> | SatResult)) => void, child : ChildProcessWithoutNullStreams) {
        ...
        stdout.on('data', (line : string) => {
            ...
            const ASSIGN_NUMBER_REGEX = /\(define-fun ([A-Za-z0-9_.]+) \(\) Real (\d+\.\d+)\)$/;
            ...
            const ASSIGN_FLOAT_REGEX = /\(define-fun ([A-Za-z0-9_.]+) \(\) Real \(\/ (\d+) (\d+)\)\)$/;
            match = ASSIGN_FLOAT_REGEX.exec(line);
            if (match !== null) {
                if (this.isPlaceholder(match[1])) return;
                assignment[match[1]] = Number.parseFloat(match[2]) / Number.parseFloat(match[3]);
                return;
            }
